...
It assumes PostgreSQL credentials are supplied through environment variables.
...
Set the datasource environment variables before starting the application:

```shell
export SPRING_DATASOURCE_URL=jdbc:postgresql://localhost:5433/store
...
Alternatively, use the startup script from the project root. It uses the local
PostgreSQL URL by default and prompts for the password when one is not already
provided:
...
For non-interactive environments, provide all datasource variables explicitly:
